    ...
    $(document).ready(function() {
        ...
        $('.selected-action ul li a').click(function() {
            ...
            swal({
                    ...
                    confirmButtonText: "{{trans('confirmation_yes')}}",
...
<form id='form-table' method='post' action='{{CRUDBooster::mainpath("action-selected")}}'>
    ...
    <table id='table_dashboard' class="table table-hover table-striped table-bordered">
        <thead>
            <tr class="active">
                <!-- CODE_BY_MAICOL :: Agregar header de enumerador Tabla th -->
                ...
                <th width='{{ isset($button_action_width)? $button_action_width :"auto"}}' style="text-align: center;"><i class='fa fa-pencil'></i></th>
                @endif
                @endif
                -->
                ...
                <th style="text-align:right">{{trans("crudbooster.action_label") }}</th>
                @endif
                @endif
        ...
        <tbody>
            ...
            <tr>
                @endif
                <!-- CODE_BY_MAICOL :: Agregar Enumerador a la tabla -->
                <td>{{$i + 1}}</td>
                @foreach($hc as $j=>$h)
...
<div class="col-md-8 col-sm-8 col-xs-8">{!! urldecode(str_replace("/?","?",$result->appends(Request::all())->render())) !!}</div>
<?php
...
$total = $result->total();
?>
<div class="col-md-4 col-sm-4 col-xs-4" style="margin: 0;"><span class="pull-right">{{ trans("crudbooster.filter_rows_total") }}
    ...
    $(function() {
        ...
        $(".toggle_advanced_report").click(function() {
            ...
            if (toggle_advanced_report_boolean == 1) {
                ...
                $(this).html("<i class='fa fa-minus-square-o'></i> {{trans('export_dialog_show_advanced')}}");
                toggle_advanced_report_boolean = 0;
            } else {
                $("#advanced_export").slideUp();
                $(this).html("<i class='fa fa-plus-square-o'></i> {{trans('export_dialog_show_advanced')}}");
                            ...
                            <div class='row-filter-combo row'>
                                ...
                                <div class='col-sm-3'>
                                    <select name='filter_column[{{$col["field_with"]}}][type]' data-type='{{$col["type_data"]}}' class="filter-combo form-control">
                                        ...
                                        <option {{ (CRUDBooster::getTypeFilter($col["field_with"]) == 'empty')?"selected":"" }} value='empty'>{{trans("crudbooster.filter_empty_or_null")}}</option>
                                    </select>
                                </div><!--END COL_SM_4-->
                                    ...
                                    <div class='row between-group' style="{{ (CRUDBooster::getTypeFilter($col["field_with"]) == 'between')?"display:block":"display:none" }}">
                                        <div class='col-sm-6'>
                                            <div class='input-group {{ ($col["type_data"] == "time")?"bootstrap-timepicker":"" }}'>
                                                ...
                                                <input {{ (CRUDBooster::getTypeFilter($col["field_with"]) != 'between')?"disabled":"" }} type='text' class='filter-value-between form-control {{ in_array($col["type_data"],["date","datetime","timestamp"]) ? "datepicker" : ((in_array($col["type_data"],["time"])) ? "timepicker" : "") }}' {{ in_array($col["type_data"],["date","datetime","timestamp","time"]) ? "readonly" : "" }} placeholder='{{$col["label"]}} {{trans("crudbooster.filter_from")}}' name='filter_column[{{$col["field_with"]}}][value][]' value='<?php
                                        ...
                                        <div class='col-sm-6'>
                                            <div class='input-group {{ ($col["type_data"] == "time")?"bootstrap-timepicker":"" }}'>
                                                ...
                                                <input {{ (CRUDBooster::getTypeFilter($col["field_with"]) != 'between')?"disabled":"" }} type='text' class='filter-value-between form-control {{ in_array($col["type_data"],["date","datetime","timestamp"]) ? "datepicker" : (in_array($col["type_data"],["time"]) ? "timepicker" : "" )}}' {{ in_array($col["type_data"],["date","datetime","timestamp","time"]) ? "readonly": "" }} placeholder='{{$col["label"]}} {{trans("crudbooster.filter_to")}}' name='filter_column[{{$col["field_with"]}}][value][]' value='<?php
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        $value = CRUDBooster::getValueFilter($col["field_with"]);
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form method='post' target="_blank" action='{{ CRUDBooster::mainpath("export-data?t=".time()) }}'>
                ...
                <div class="modal-footer" align="right">
                    <button class="btn btn-default" type="button" data-dismiss="modal">{{trans("crudbooster.button_close")}}</button>
                    <button class="btn btn-primary btn-submit" type="submit">{{trans('button_submit')}}</button>
                </div>
            </form>
        </div>
        <!-- /.modal-content -->
