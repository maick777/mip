<div class='form-group form-datepicker {{$header_group_class}} {{ ($errors->first($name))?"has-error":"" }}' id='form-group-{{$name}}' style="{{@$form['style']}}">
    ...
    <div class="{{$col_width?:'col-sm-10'}}">
        <div class="input-group">
            ...
            <!-- CODE_BY_MAICOL :: Cambiar datepicker
            ...
            <input type='text' title="{{$form['label']}}" {{$required}} {{$readonly}} {!!$placeholder!!} {{$disabled}} class='form-control notfocus' name="{{$name}}" id="{{$name}}" value='{{$value}}' />
