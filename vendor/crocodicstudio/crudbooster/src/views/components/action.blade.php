@foreach($addaction as $a)
    ...
    if (isset($a['showIf'])) {
        ...
        @eval("if($query) {
          echo \"<a class='btn btn-xs btn-\$color' data-toggle='tooltip' title='\$title' onclick='\$confirm_box' href='\$url' target='\$target'><i class='\$icon'></i> $label</a>&nbsp;\";
      }");
    } else {
        echo "<a class='btn btn-xs btn-$color'data-toggle='tooltip' title='$title' onclick='$confirm_box' href='$url' target='$target'><i class='$icon'></i> $label</a>&nbsp;";
    }
...
@if($button_action_style == 'button_text')
    ...
    @if(CRUDBooster::isDelete() && $button_delete)
        ...
        <a class='btn btn-xs btn-warning btn-delete'  href='javascript:;'
...
@elseif($button_action_style == 'dropdown')
    ...
    <div class='btn-group btn-group-action'>
        ...
        <ul class='dropdown-menu dropdown-menu-action' role='menu'>
            ...
            @if(CRUDBooster::isDelete() && $button_delete)
                <?php $url = CRUDBooster::mainpath("delete/".$row->$pk);?>
                <li><a class='btn-delete'  href='javascript:;'
...
@else
    ...
    @if(CRUDBooster::isRead() && $button_detail)
        <a class='btn btn-xs btn-primary btn-detail' data-toggle="tooltip" title='{{trans("crudbooster.action_detail_data")}}'
           href='{{CRUDBooster::mainpath("detail/".$row->$pk)."?return_url=".urlencode(Request::fullUrl())}}'><i class='fa fa-eye'></i></a>
    @endif

    @if(CRUDBooster::isUpdate() && $button_edit)
        <a class='btn btn-xs btn-success btn-edit' data-toggle="tooltip" title='{{trans("crudbooster.action_edit_data")}}'
           href='{{CRUDBooster::mainpath("edit/".$row->$pk)."?return_url=".urlencode(Request::fullUrl())."&parent_id=".g("parent_id")."&parent_field=".$parent_field}}'><i
    ...
    @if(CRUDBooster::isDelete() && $button_delete)
        <?php $url = CRUDBooster::mainpath("delete/".$row->$pk);?>
        <a class='btn btn-xs btn-warning btn-delete' data-toggle="tooltip" title='{{trans("crudbooster.action_delete_data")}}' href='javascript:;'
