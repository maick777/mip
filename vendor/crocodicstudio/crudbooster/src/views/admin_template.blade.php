...
<head>
    ...
    <link rel="shortcut icon" href="{{ CRUDBooster::getSetting('favicon')?asset(CRUDBooster::getSetting('favicon')):asset('vendor/crudbooster/avatar.png') }}">
    ...
    <link rel='stylesheet' href='{{asset("vendor/crudbooster/assets/css/animate.css")}}' />
        ...
        <div class="content-wrapper">
            ...
            <section class="content-header">
                ...
                <h4>
                    ...
                    <a href='{{$ib["url"]}}' id='{{str_slug($ib["label"])}}' class='btn {{($ib['color'])?'btn-'.$ib['color']:'btn-primary'}} btn-sm' @if($ib['onClick']) onClick='return {{$ib["onClick"]}}' @endif @if($ib['onMouseOver']) onMouseOver='return {{$ib["onMouseOver"]}}' @endif @if($ib['onMouseOut']) onMouseOut='return {{$ib["onMouseOut"]}}' @endif @if($ib['onKeyDown']) onKeyDown='return {{$ib["onKeyDown"]}}' @endif @if($ib['onLoad']) onLoad='return {{$ib["onLoad"]}}' @endif>
            ...
            <section id='content_section' class="content">
                ...
                <script>
                    ...
                    function getApiProvincias(ctn, id) {
                        let name = $(ctn);
                        if (!name && !id) {
                            alerta("error", 'No se pudo obtener la información');
                            return false;
                        }
                        name.html("");
                        name.append("<option value=''>Cargando...</option>");

                        $.ajax({
                            url: `{{ asset('api') }}/provincias/${id}`,
                            method: 'get',
                            dataType: 'json',
                            success: function(response) {
                                name.html("");
                                name.append("<option value=''>** Seleccione una Provincia</option>");
                                ...
                                if (provincias.length > 0) {
                                    ...
                                    alerta("success", "Se cargo las Provincias");

                                } else {
                                    alerta("error", 'No se pudo obtener información solicitada!.');
                                }
                            }
                        });
                    ...
                    function getApiDistritos(ctn, id) {
                        let name = $(ctn);
                        if (!name && id) {
                            alerta("error", 'No se pudo obtener información');
                            return false;
                        }
                        name.html("");
                        name.append(`<option value=''>Cargando...</option>`);
                        ...
                        if (id != '') {
                            $.ajax({
                                url: `{{ asset('api') }}/distritos/${id}`,
                                method: 'get',
                                dataType: 'json',
                                success: function(response) {
                                    name.html("");
                                    name.append("<option value=''>** Seleccione un Distrito</option>");
                                    ...
                                    if (distritos.length > 0) {
                                        ...
                                        alerta("success", "Se cargo los Distritos");

                                    } else {
                                        alerta("error", 'No se pudo obtener la información solicitada!.');
                                    }
                                }
                            });
                        }
