...
use crocodicstudio\crudbooster\helpers\CB as CB;
...
class CBController extends Controller
{
    ...
    public function getIndex()
    {
        ...
        $fields_search = [];
        foreach ($this->fields_search as $i => $f) {
            $key = array_search($f, array_column($columns_table, 'name'));
            if ($key) {
                $fields_search[] = $columns_table[$key];
            }
        }

        if (Request::get('q')) {
            $request = '';
            $result->where(function ($w) use ($columns_table, $fields_search, $request) {
                if (count($fields_search)) {
                    foreach ($fields_search as $col) {
                        if (!$col['field_with']) {
                            continue;
                        }
                        if ($col['is_subquery']) {
        ...
        foreach ($data['result'] as $row) {
            ...
            /*
            //CODE_BY_MAICOL :: Agregar botones al inicio
            if ($this->button_table_action) :

                if(CRUDBooster::isRead()){
                    $button_action_style = $this->button_action_style;
                    $html_content[] = "<div class='button_action' style='text-align:center'>" . view('crudbooster::components.action_edit', compact('addaction', 'row', 'button_action_style', 'parent_field'))->render() . "</div>";
                }
            ...
            */
            ...
            foreach ($columns_table as $col) {
                ...
                if (isset($col['image'])) {
                    if ($value == '') {
                        $value = "<a  data-lightbox='roadtrip' rel='group_{{$table}}' title='$label: $title' href='" . asset('vendor/crudbooster/avatar.png') . "'><img width='20px' height='20px' src='" . asset('vendor/crudbooster/avatar.png') . "'/></a>";
                    } else {
                        $pic = (strpos($value, 'http://') !== false) ? $value : asset($value);
                        $value = "<a data-lightbox='roadtrip'  rel='group_{{$table}}' title='$label: $title' href='" . $pic . "'><img width='20px' height='20px' src='" . $pic . "'/></a>";
                    }
                }
    ...
    public function postDoneImport()
    {
        $this->cbLoader();
        $data['page_menu'] = Route::getCurrentRoute()->getActionName();
        $data['page_title'] = trans('crudbooster.button_import');
