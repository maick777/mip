...
class CRUDBooster
{
    ...
    public static function uploadFile($name, $encrypt = false, $resize_width = null, $resize_height = null, $id = null)
    {
        if (Request::hasFile($name)) {
            ...
            // CODE_BY_MAICOL :: Cambiar nombre del directorio
            ...
            if ($encrypt == true) {
                ...
                // CODE_BY_MAICOL :: Cambiar nombre de archivo
                $filename =  date('Ymd_His') . $userID . rand(0, 9) . '.' . $ext;
            }
    ...
    */
    ...
    public static function myLogotipo()
    {
        return Session::get('admin_logotipo');
    }

    public static function appAvatar()
    {
        return Session::get('app_avatar');
    }
    ...
    // CODE_BY_MAICOL :: Agregar Sede

    public static function myYacimientoId()
    {
        $query = DB::table('cms_users')->where('id', '=', self::myId())->first();
        return $query->id_yacimiento;
    }

    public static function myYacimientoName()
    {
        $query = DB::table('yacimientos')->where('id', '=', self::mySedeId())->first();

        if (self::myNivel() == 'General') {
            return  'GENERAL';
        } else {
            return $query->nombre_corto;
        }
    }


    public static function myNivel()
    {
        return Session::get('admin_privileges_nivel');
    }
    ...
    // CODE_BY_MAICOL ::Agregar opciones de permiso descarga y reporte gráfico
